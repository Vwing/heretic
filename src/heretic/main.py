...
def get_lora_adapter_metadata(
    ...
    method = "ara_lora_approximation" if settings.use_ara else "directional_abliteration"
    notes = (
        "This is a PEFT LoRA adapter generated by approximating Heretic's ARA "
        "weight edit with truncated SVD. Load it on top of the base_model; "
        "do not treat it as a standalone model."
        if settings.use_ara
        else (
            "This is a PEFT LoRA adapter generated by Heretic's non-ARA "
            "directional ablation path. Load it on top of the base_model; "
            "do not treat it as a standalone model."
        )
    )

    metadata = {
        "format": "heretic_lora_adapter",
        "format_version": 1,
        ...
        "trial": {
            ...
            "values": list(trial_values) if trial_values is not None else None,
            "direction_index": trial.user_attrs.get("direction_index"),
            "parameters": trial.user_attrs.get("parameters"),
            "ara_parameters": trial.user_attrs.get("ara_parameters"),
        },
...
def save_lora_adapter_metadata(
    ...
    metadata_path = path / ADAPTER_METADATA_FILENAME
    metadata_path.write_text(
        json.dumps(
            get_lora_adapter_metadata(settings, trial, adapter_metadata),
            indent=2,
            sort_keys=True,
        )
        + "\n",
...
def prepare_lora_adapter_for_export(model: Model, settings: Settings) -> None:
    # PEFT's adapter_config.json has a standard base_model_name_or_path field.
    # Keep Heretic's richer metadata separate, but also populate the standard field
                            ...
                            if strategy == "adapter":
                                ...
                                if settings.use_ara and not model.has_lora_adapter():
                                    if ara_original_weights is None:
                                        raise Exception(
                                            "Cannot export ARA adapter without original weight snapshot."
                                        )
                                    print(
                                        f"* Compressing ARA edit to rank-{settings.ara_lora_rank} LoRA..."
                                    )
                                    ara_adapter_metadata = (
                                        model.convert_ara_to_lora_adapter(
                                            ara_original_weights,
                                            settings.ara_lora_rank,
                                        )
                                    )
                        ...
                        case "Upload the model to Hugging Face":
                            ...
                            if strategy == "adapter":
                                ...
                                with tempfile.TemporaryDirectory() as temp_dir:
                                    metadata_path = save_lora_adapter_metadata(
                                        temp_dir,
                                        settings,
                                        trial,
                                        ara_adapter_metadata,
                                    ...
                                    huggingface_hub.upload_file(
                                        ...
                                        path_in_repo=ADAPTER_METADATA_FILENAME,
                                        repo_id=repo_id,
                                        token=token,
                                    )
                            else:
