...
def obtain_merge_strategy(settings: Settings) -> str | None:
    ...
    if not settings.use_ara:
        choices.append(
            Choice(
                title="Export LoRA adapter only (small, requires the original base model)",
                value="adapter",
            )
        )
...
def get_lora_adapter_metadata(settings: Settings, trial: Trial) -> dict[str, Any]:
    ...
    return {
        ...
        "method": "directional_abliteration",
        "notes": (
...
def save_lora_adapter_metadata(
    ...
    metadata_path.write_text(
        json.dumps(
            get_lora_adapter_metadata(settings, trial),
            indent=2,
            sort_keys=True,
        )
        + "\n",
